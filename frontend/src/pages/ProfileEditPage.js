...
function ProfileEditPage() {   
    ...
    useEffect(()=>{  
      const getUserInfo = async () => {   
          try{  
            const response = await axios.post("http://localhost:5000/user/getUser", { id: id });   
    ...
    const handleUpdate = async (e) => { 
        e.preventDefault(); 
        try { 
            const response = await axios.post("http://localhost:5000/user/updateProfile", {id,user_email,userName,firstName,lastName,bio,birthday,gender});  
